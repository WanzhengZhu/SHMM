...
public class Multinomial implements Serializable {
	...
	public void fit(int dimension, List<Map<Integer, Integer>> data, List<Double> weights) {
	...
	private void learnProb(List<Map<Integer, Integer>> data, List<Double> weights) {
		for (int i = 0; i < data.size(); i++) {
			Map<Integer, Integer> obs = data.get(i);
	...
	public double calcLL(Map<Integer, Integer> sample) {
		return calcLL(sample, false);
	}

	public double calcLL(Map<Integer, Integer> sample, boolean isTest) {
		double result = 0.0;
		int totalCount = 0;
